...
BOOL CEditDialog::OnInitDialog ()
{
    ...
    SetDlgItemText(IDC_MOD
                    , m_pData -> GetModTime().Format("%m/%d/%y %H:%M"));
...
CMainWindow::CMainWindow ()
{
    ...
    VERIFY(Create(NULL, "Phone Book", WS_OVERLAPPEDWINDOW, rectDefault
                    , NULL, "MainMenu"));
    m_nSelectLine =  - 1 ;
}
...
void CMainWindow::OnPrint ()
{
    ...
    if ( PrintDlg(&pd) != 0 ) {
        ...
        int nExtPage
                 = (dc -> GetDeviceCaps(VERTRES) - nCharHeight)
                     / nCharHeight ;
        CString szTitle ;
        ...
        while ( nPerson != m_people.GetCount()) {
            ...
            dc -> LineTo(dc -> GetTextExtent(szTitle, szTitle.GetLength())
                                .cx
                            , nCharHeight);
...
void CMainWindow::OnSize ( UINT, int x, int y )
{
    m_cxClient = x ;
    m_cyClient = y ;
    m_nVscrollMax = max(0, (int)(m_people.GetCount())
                             - m_cyClient / m_cyChar);
...
void CMainWindow::OnPaint ()
{
    ...
    int nEnd = min((int)m_people.GetCount()
                    , m_nVscrollPos
                         + (dc.m_ps.rcPaint.bottom / m_cyChar + 1));
    ...
    for (; nStart < nEnd ; nStart++ ) {
        ...
        // Write the time.  
        szDisplay = pCurrent -> GetModTime().Format("%m/%d/%y %H:%M");
        dc.TextOut(x + (SIZENAME + SIZEPHONE) * m_cxCaps, y, szDisplay
                    , szDisplay.GetLength());
...
BOOL CMainWindow::CheckForSave ( const char *pszTitle
