...
const vector<std::string>   doxyStart = {"@fn", "@buf", "@file", "@param", "@returns", "@brief", "@class"};
...
#   endif
    ...
    if ( !nameReferenced ) {
        ...
        if ( tree == <FUNC,rangeModif,type,declarator,param> ) {
            ...
            PTREE           newDeclarator = <TYP_LIST,copytree(theDeclarator),copytree(param),(),()>;
    ...
    if ( returnFunct == () ) {
        PTREE   type ;
        if ( tree == <FUNC,<>,type> && type != <VOID> && type != () ) {
...
void DoxyClassNormalize ( PTREE &tree, DoxyContent &doxy, PTREE &className )
{
...
void AdjustNL ( EString &commentString )
{
    EString adjustment("\n");
    
    adjustment << "/" << doxyStarter ;
...
void DoxyClass ( PTREE &tree, PTREE &classTree )
{
    ...
    classTree == <CLASS,<>,name>;
    
    // reconstruct the different elems
    DoxyClassNormalize(tree, doxyContent, name);
            ...
            case <CLASS> : 
                {
                    ...
                    PTREE   classTree (elem) ;
                    while ( classTree != tree && classTree != <DECLARATION> ) {
                        classTree =  classTree ^ ;
                    }
                    if ( classTree == <DECLARATION> ) 
                        DoxyClass(classTree, elem);
                }
                break ;
            default : break ;
        }
