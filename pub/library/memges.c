...
static long         posBuffer [BUFFER_NUMBER];                /* the beginning */ /* position of */ /* each cache */ 
static long         timeBuffer [BUFFER_NUMBER];               /* time of last */ /* use */ 
static char         writeBuffer [BUFFER_NUMBER];
static char         contBuffer [BUFFER_NUMBER][BUFFER_SIZE];  /* the cache */ 
static int          currentBuffer = 0 ;
static int          cacheFile ;                               /* the file used */ /* by the cache */ /* manager */ 
...
int CacheRead ( long position )
{
    ...
    deb : 
        ...
        if ( position >= cachePosition && position <= cachePosition + BUFFER_SIZE - 1 ) {
...
BufferPosition ( long position )
{
    ...
    if ( writeBuffer [currentBuffer] ) {
        ...
        /* save current data */
        if ( posBuffer [currentBuffer] >= 0 && posBuffer [currentBuffer] < maxpos ) {
            lseek(cacheFile, posBuffer [currentBuffer], SEEK_SET);
            if ( write(cacheFile, (char *)contBuffer [currentBuffer], BUFFER_SIZE) == -1 ) {
    ...
    /* read the new data */
    if ( (posBuffer [currentBuffer] = (long)position / BUFFER_SIZE * BUFFER_SIZE) > maxpos ) {
...
void CacheInit ()
{
    ...
    cacheFile =  open(TMP_NAME, O_CREAT | O_RDWR | O_BINARY, S_IREAD | S_IWRITE);
...
void CacheFree ( long position )
{
    ...
    if ( ptOld != ptFreeElem && position < ptOld -> deb + ptOld -> size || position + size > ptFreeElem -> deb ) {
        write(2, "double free \n", 13);
        return ;
    }
    if ( ptFreeElem ) 
