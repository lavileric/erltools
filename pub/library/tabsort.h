...
#               endif
            ...
            virtual int GetIndex ( const void *elem, int min = 0, int max = -10 )
            {
                if ( !elem ) 
                    return -10 ;
                
                const char  *string = GetElemName(elem);
                
                if ( !string ) 
                    return -10 ;
                
                char    *aString = AllocString(string);
                
                if ( !aString ) 
                    return -10 ;
                
                int retValue = GetIndex(aString, min, max);
            ...
            virtual void    InsertRemove (void *elem, int min = 0, int max = -10) ;
    ...
    template <class Content> 
        ...
        class TabSortEqual : public TabSortRoot {
            ...
            public :
                ...
                virtual Content *GetFirst ( const Content *elem )
                {
                    return (Content *)GetFirst((const void *)elem);
                }
                
                virtual Content *GetNext ( int index, const Content *elem )
                {
                    void    *result = GetNext(index, (const void *)elem);
                    
                    return (Content *)result ;
                }
                
                virtual Content *GetFirstIndex ( const Content *elem )
                {
                    return (Content *)TabSortRoot::Get(GetFirstIndex((const void *)elem));
                }
                
                virtual Content *GetNextIndex ( int index, const Content *elem )
                {
                    int nextIndex = GetNextIndex(index, (const void *)elem);
                    
                    return (Content *)TabSortRoot::Get(nextIndex);
                }
                
                virtual void *GetFirst ( const void *elem )
                {
                    return TabSortRoot::Get(GetFirstIndex(elem));
                }
                
                virtual void *GetNext ( int index, const void *elem )
                {
                    return TabSortRoot::Get(GetNextIndex(index, elem));
                }
                
                virtual void *GetFirst ( const char *elem )
                {
                    return TabSortRoot::Get(GetFirstIndex(elem));
                }
                
                virtual void *GetNext ( int index, const char *elem )
                {
                    return TabSortRoot::Get(GetNextIndex(index, elem));
                }
                
                virtual int GetFirstIndex ( const void *elem )
                {
                    if ( !elem ) 
                        return -10 ;
                    ...
                    int retValue = GetFirstIndex(aString);
                    
                    free(aString);
                    return retValue ;
                }
                
                virtual int GetNextIndex ( int index, const void *elem )
                {
                    if ( !elem ) 
                        return -10 ;
                    ...
                    int retValue = GetNextIndex(index, aString);
                    
                    free(aString);
                    return retValue ;
                }
                
                virtual int GetFirstIndex ( const char *elem, int min = 0, int max = -10 )
                {
                    
                    // get index 
                    int index = GetIndex(elem, min, max);
                    ...
                    while ( index ) {
                        if ( index <= 0 || index <= min ) 
                            break ;
                        {
                            if ( !strcmp(GetElemName(*(array + index - 1)), elem) ) 
                                index =  index - 1 ;
                            else 
                                break ;
                        }
                ...
                virtual int GetNextIndex ( int refIndex, const char *elem, int max = -10 )
                {
                    ...
                    // got to first
                    if ( index >= 0 && index < Size() && !strcmp(GetElemName(*(array + index)), elem) ) {
                        return index ;
                    } else 
                        return -10 ;
                    return index ;
...
#               ifndef SOLARIS_CC
                    ...
                    virtual int GetIndex ( const void *elem, int min = 0, int max = -10 )
                    {
                        return TabSortRoot::GetIndex((const void *)elem, min, max);
                    }
                    
                    virtual int GetIndex ( const char *name, int min = 0, int max = -10 )
                    {
                    ...
                    // inserting an element in the list
                    virtual void Insert ( Content *elem, int min = 0, int max = -10 )
                    {
                    ...
                    // inserting an element in the list
                    // if element is allready there remove the old
                    virtual void InsertRemove ( Content *elem, int min = 0, int max = -10 )
                    {
