...
public class SimplCon {
    ...
    static public void main(String args[]) {
        ...
        if ( false ) {
            ...
            Lock lock = new Lock ();
            PackApi packApi = new PackApi("pack","127.0.0.1","127.0.0.1",2000,lock);
            ...
            packApi.Send(lParam.Reset().Push("load").Push("Telnetmanager.isl"));
            
            // ask to start scenario
            packApi . Send(lParam . Reset().Push("start").Push("TelnetManager").Push(ipAddress));
            ...
            while (true) {
                ...
                if (content.size() >0 ) {
                    ...
                    if ( type . compareTo("data") == 0 ) {
                        ...
                        String send = ((Param)(content . elementAt(1))) . pvsData ;
             ...
             Lock lock = new Lock();
             TelnetTester cubitus = new TelnetTester("cubitus","172.20.64.136","guest","guest",lock);
             cubitus.start();
             TelnetTester scdis = new TelnetTester("guest","172.20.64.136","guest","guest",lock);
             scdis.start();
             TelnetTester cubitus1 = new TelnetTester("cubitus1","172.20.64.136","guest","guest",lock);
             cubitus1.start();
             TelnetTester scdis1 = new TelnetTester("scdis1","172.20.64.136","guest","guest",lock);
             scdis1.start();
             ...
             TelnetTester cubitus2 = new TelnetTester("cubitus2","172.20.64.136","guest","guest",lock);
             cubitus2.start();
             TelnetTester scdis2 = new TelnetTester("scdis2","172.20.64.136","guest","guest",lock);
             scdis2.start();             
             TelnetTester cubitus3 = new TelnetTester("cubitus3","172.20.64.136","guest","guest",lock);
             cubitus3.start();
